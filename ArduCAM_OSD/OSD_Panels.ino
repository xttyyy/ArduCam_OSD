...
void writePanels(){
	...
	if (millis() < (lastMAVBeat + 2200))
	{
		...
		if (c_configuring == 1)
		{
			//osd.setPanel(7, 4);
			//osd.openPanel();
			//osd.printf_P(PSTR("c_configuring"));
			//osd.closePanel();
			//panConfigure();
			panSetup2();
		}
...
void panSetup2()
{
	...
	if (ms > text_timer)
	{
		...
		////osd.printf("%s", menus[7]);
		//osd.printf("pansetup2:%6i",ms);
		//osd.closePanel();
		...
		for (i = 0; i < num; i++)
		{
			...
			for (int j = 0; j < 8; j++)
				words[j] = EEPROM.read(221 + num + i * 8 + j);
			osd.printf("%s", words);
			osd.closePanel();
		}*/
		int num = EEPROM.read(220);
		...
		if (parrent == -1)
		{
			...
			for (i = 0; i < num; i++)
			{
				...
				char rowhead = '\x00';
				if (i == rowDeeps)
					rowhead = '\xba';   //0xba is a right arrow, 0xbb is a updown arrow

				for (int j = 0; j < 8; j++)
					words[j] = EEPROM.read(221 + num + i * 8 + j);
				osd.printf("%c%s", rowhead, words);
				osd.closePanel();
			}
...
/*
		osd.setPanel(1, 1 + i);
		osd.openPanel();
		osd.printf("c1raw: %i", chan1_raw);
		osd.closePanel();

		osd.setPanel(1, 2 + i);
		osd.openPanel();
		osd.printf("c1mid: %i", c_chan1_middle);
		osd.closePanel();

		osd.setPanel(1, 3 + i);
		osd.openPanel();
		osd.printf("c1rev: %i", c_chan1_rev);
		osd.closePanel();

		osd.setPanel(16, 1 + i);
		osd.openPanel();
		osd.printf("c2raw: %i", chan2_raw);
		osd.closePanel();

		osd.setPanel(16, 2 + i);
		osd.openPanel();
		osd.printf("c2mid: %i", c_chan2_middle);
		osd.closePanel();

		osd.setPanel(16, 3 + i);
		osd.openPanel();
		osd.printf("c2rev: %i", c_chan2_rev);
		osd.closePanel();

		osd.setPanel(1, 4 + i);
		osd.openPanel();
		osd.printf("c1mtime: %i", c_last_chan1_move_time);
		osd.closePanel();

		osd.setPanel(16, 4 + i);
		osd.openPanel();
		osd.printf("isconf: %i", c_configuring);
		osd.closePanel();
		...
		osd.setPanel(1, 6 + i);
		osd.openPanel();
		osd.printf("xDeeps: %i", menuDeeps);
		osd.closePanel();

		osd.setPanel(16, 6 + i);
		osd.openPanel();
		osd.printf("yDeeps: %i", rowDeeps);
		osd.closePanel();

		osd.setPanel(1, 7 + i);
		osd.openPanel();
		osd.printf("parrent: %i", parrent);
		osd.closePanel();*/
		//	for (int i = 0; i < 10; i++)
		//	{
		//		if (parrent == -1)  		
...
void panConfigure()
{
	...
	if (ms > text_timer)
	{
		...
		osd.setPanel(1, 8);
		osd.openPanel();
		osd.printf("rowDeeps: %i", rowDeeps);
		osd.closePanel();
		//osd.closePanel();
